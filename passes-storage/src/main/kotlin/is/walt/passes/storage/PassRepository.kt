...
public interface PassRepository {
    ...
    /**
     * Loads all persisted artifacts in one shot for wallet export. The returned
     * [ExportSnapshot] contains fully materialised [StoredPass] objects (images and
     * locales included), all [ScannableCard] rows, and every [DocumentExportEntry] with
     * its raw PDF bytes already fetched from the `documents` table.
     *
     * Documents whose bytes cannot be retrieved (e.g. a row whose blob was lost to a
     * partial write) are silently skipped so one corrupt row does not abort the entire
     * export.
     *
     * The snapshot is a point-in-time read; no write-mutex is held across it. Concurrent
     * writes during export are benign: new rows appear in the next export, not this one.
     */
    public suspend fun readAllForExport(): StorageResult<ExportSnapshot>
...
public data class StoredPass(
    ...
    override val exportKind: String get() = ArtifactKind.PKPASS
    override val exportId: String get() = pass.serialNumber
...
/**
 * Point-in-time snapshot of all wallet artifacts, produced by [PassRepository.readAllForExport].
 * Handed to the export layer (walt-android → `passes-export`) to assemble a [WalletExportPayload].
 */
public class ExportSnapshot(
    public val passes: List<StoredPass>,
    public val scannableCards: List<ScannableCard>,
    public val documents: List<DocumentExportEntry>,
)

/**
 * A [DocumentRow] paired with the raw PDF bytes needed to populate [ArtifactEnvelope.blob].
 * The thumbnail is intentionally excluded — it is a derived render artifact and must be
 * regenerated by the importer via the PDF renderer service.
 */
public class DocumentExportEntry(
    public val row: DocumentRow,
    public val pdfBytes: ByteArray,
)
