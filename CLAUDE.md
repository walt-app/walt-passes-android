...
- Eleven Gradle modules:
  ...
  - `passes-export-core` — pure Kotlin/JVM. `ExportableArtifact` interface and `ArtifactKind` constants. Zero dependencies within this repo. Both `passes-core` and `passes-pdf-core` depend on it (`api`) so that domain objects (`ScannableCard`, `PdfDocument`) implement `ExportableArtifact` directly. NO Android framework dependencies.
...
- `passes-core`, `passes-pdf-core`, `passes-export-core`, and `passes-barcode-core` have NO Android framework dependencies (KMP-friendly).
