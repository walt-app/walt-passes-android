...
- Thirteen Gradle modules:
  ...
  - `passes-ui-core` — Android + Compose. Tiny shared substrate for both UI modules: `ArgbColor`, `Color` conversion, the `isolated()` BiDi (FSI/PDI) helper, and the `faceIsTinted` face-tint predicate (the shared gate deciding whether a consumer-supplied face tint is taken; `isSpecified` alone is true for `Color.Transparent`). NO trust-claim-bearing surfaces of its own; existence is to keep `passes-ui` and `passes-document-ui` from depending on each other for these primitives.
