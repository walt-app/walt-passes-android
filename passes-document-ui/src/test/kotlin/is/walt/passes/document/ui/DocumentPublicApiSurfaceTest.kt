...
/**
 * Locks the public API surface of `passes-document-ui`. Mirrors `passes-ui`'s and
...
class DocumentPublicApiSurfaceTest {
    ...
    /**
     * Bytecode scan that fails closed if any compiled `is.walt.passes.document.ui.*` class
                ...
                if (index >= 0) {
                    error(
                        "Forbidden $label string '$needle' found at offset $index in " +
                            ...
                            "0005 D4 forbids PDF MIME / metadata surfaces in passes-document-ui. " +
