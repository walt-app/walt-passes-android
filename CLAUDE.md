...
- `passes-storage`, `passes-pdf`, `passes-barcode`, `passes-image`, `passes-ui`, and `passes-pdf-ui` are independent peers (no edges among them at this level). The single permitted exception is `passes-pdf-ui → passes-pdf`, since the document-rendering surface needs `PdfRendererBinder` to take pages from. (The analogous `→ passes-image` edge for the image-document display surface lands with the wpass-i9x UI step, not here.)
...
- `passes-pdf`, `passes-barcode`, and `passes-image` all depend on `passes-isolation` for the shared bind/memfd plumbing. `passes-isolation` depends on nothing in this repo (only the Android framework + coroutines), so it adds no edge among those peers — it sits below all three. No other module depends on it.
