...
/**
 * Pins both manifest security invariants on `passes-image`'s source manifest, making the
 * manifest's headline security claims structural rather than aspirational (the same discipline
 * [PublicApiSurfaceTest] applies to the facade surface), mirroring `passes-barcode` /
 * `passes-pdf`:
 *
 ...
 *  2. The decode service is declared `isolatedProcess="true"`, `exported="false"`, and names
 *     exactly the [ImageDecodeService] class in its own `:imageDecoder` process.
 *
...
class ManifestPermissionsTest {
    ...
    fun decodeServiceIsIsolatedAndNotExported() {
        ...
        assertThat(svc.getAttributeNS(ANDROID_NS, "name")).isEqualTo(".android.ImageDecodeService")
        assertThat(svc.getAttributeNS(ANDROID_NS, "process")).isEqualTo(":imageDecoder")
    }

    private fun manifestFile(): File {
        val moduleDir = System.getProperty("walt.passes.image.moduleDir") ?: "."
