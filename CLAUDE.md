...
- `passes-barcode` and `passes-ui` both depend on `passes-image-decode` for the shared bounded `ImageDecoder` mechanism. `passes-image-decode` depends on nothing in this repo (only `android.graphics`), so it adds no edge between those two — it sits below both, like `passes-isolation`. The only other consumer is the future wpass-i9x isolated image-document service.
