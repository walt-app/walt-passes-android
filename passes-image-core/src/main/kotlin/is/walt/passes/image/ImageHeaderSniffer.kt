...
/**
 * Header-sniff for the three accepted image formats. Returns the detected [ImageFormat],
 * or `null` if the byte sequence does not match any supported magic.
 *
 * Each format is identified by its leading magic bytes:
 *
 *  - **PNG**: first 8 bytes are `89 50 4E 47 0D 0A 1A 0A` (the PNG signature).
 *  - **JPEG**: first 3 bytes are `FF D8 FF` (SOI marker + next-marker prefix). The
 *    fourth byte identifies the specific JFIF/EXIF/raw variant; it is intentionally
 *    not checked here — any `FF D8 FF` start is a valid JPEG preamble.
 *  - **WebP**: bytes 0–3 are `52 49 46 46` ("RIFF") and bytes 8–11 are
 *    `57 45 42 50` ("WEBP"). Bytes 4–7 are the file-size field and are skipped.
 *    Requires at least 12 bytes.
 *
 * All checks are anchored to the very first byte. Searching for the magic elsewhere in
 * the buffer (as some lenient parsers do) would let an attacker prepend an arbitrary
 * payload before the image data; anchoring collapses that surface.
 *
 * This gate runs *before* any decoding work, so a MIME-spoofed file (PDF, ZIP,
 * executable) is rejected with [ImageRejectedKind.NotAnImage] without ever entering the
 * image decoder.
 */
